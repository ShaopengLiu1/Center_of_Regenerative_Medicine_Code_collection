...
def count(chr, dd_bed, dd_peak, output):
    ...
    for (x, y) in dd_bed:
        intx = int(x)
        inty = int(y)
        if (intx > int(dd_peak[index_peak][0]) and intx < int(dd_peak[index_peak][1])) or \
           (inty > int(dd_peak[index_peak][0]) and inty < int(dd_peak[index_peak][1])):
            reads[index_peak] += 1
        else:
            if (index_peak<n-1) and ((intx > int(dd_peak[index_peak+1][0]) and intx < int(dd_peak[index_peak+1][1])) or \
            (inty > int(dd_peak[index_peak+1][0]) and inty < int(dd_peak[index_peak+1][1]))):
                index_peak += 1
                reads[index_peak] += 1
    with open(output, 'a') as f:
